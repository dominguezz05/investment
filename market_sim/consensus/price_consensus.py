...
import numpy as np
from market_sim.consensus.dolev_strong import Node, simulate_consensus

from market_sim.consensus.dolev_strong_with_delay import simulate_consensus_with_delay
...
class NetworkPriceConsensus:
    ...
    def run_consensus(self, symbol: str, current_time: datetime) -> Optional[Decimal]:
        """Run consensus protocol for a symbol's price without delay."""
        ...
        return self._process_consensus_result(symbol, current_time, consensus_results)

    def run_consensus_with_delay(self, symbol: str, current_time: datetime, delta_ms: int) -> Optional[Decimal]:
        """Run consensus protocol with simulated network delay (∆)."""
        sender_estimate = self.nodes[0].compute_price_estimate(symbol)
        if sender_estimate is None:
            return None

        consensus_results = simulate_consensus_with_delay(
            n=self.num_nodes,
            f=self.fault_tolerance,
            sender_input=sender_estimate,
            corrupt_nodes=self.corrupt_nodes,
    ...
    def _process_consensus_result(self, symbol: str, current_time: datetime, consensus_results: List[int]) -> Optional[Decimal]:
        """Internal helper to finalize consensus and update state."""
        if consensus_results and all(r == consensus_results[0] for r in consensus_results):
            consensus_value = Decimal(consensus_results[0]) / 100
            consensus_result = {
